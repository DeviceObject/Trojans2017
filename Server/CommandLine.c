...
void ParseCommandLine(PCHAR pCmdLineA,PCOMMAND_LINE_PACK *ppCmdLinePack)
{
    ...
    do 
    {
        pLocalDat = VirtualAlloc(NULL, \
            __STRLEN__(pCmdLineA), \
    ...
    if (pLocalDat)
    {
        VirtualFree(pLocalDat,__STRLEN__(pCmdLineA),MEM_DECOMMIT | MEM_RESERVE);
    }
    return;
}
...
BOOLEAN ProcessCommandLine(PCHAR pCmdLineA,PCOMMAND_LINE_PACK *pRetCmdLinePack)
{
    ...
    *pRetCmdLinePack = NULL;
    do 
    {
        *pRetCmdLinePack = VirtualAlloc(NULL, \
            sizeof(COMMAND_LINE_PACK), \
            MEM_RESERVE | MEM_COMMIT, \
            PAGE_READWRITE);
